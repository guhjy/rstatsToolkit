...
##' @param levels.ylab Same as the \code{levels.xlab} arg, but for the
##' y-axis
...
##' @param corr.values Set whether to have the correlation values on
##' the heatmap, or if it will be blank. Value is either TRUE or
##' FALSE
##' @param leg.title.size Size of the font in the legend title
##' @param leg.number.size Size of the numbers in or near the legend
...
heatmapCorr <- function(data, x, y, leg.range=c(-1.0, 1.0),
                        ...
                        matrix.sty=FALSE, corr.values = TRUE,
                        leg.title.size = 8, leg.number.size = 7,
                        axis.text.size = 10) {
    ...
    p <- p + theme(axis.text.x = element_text(angle = 45,
                       vjust = 1, hjust = 1, size = axis.text.size),
                   ...
                   legend.text = element_text(size = leg.number.size))
